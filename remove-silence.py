...
def preprocess_audio(args):
    divided = 0
    for filepath in tqdm.tqdm(sorted(glob.glob(os.path.join(args.input_dir,args.glob)))):# 
        ...
        for i, segment in reversed(list(enumerate(segments))):
            if i >= 1:
                ...
                print('the file', output_path, ' have two segments verify this file is correct')
                divided += 1
            else:
                if flag:
    ...
    print("Num file divided:",divided)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-g', '--glob', type=str, default='wav48/*/*.wav',
